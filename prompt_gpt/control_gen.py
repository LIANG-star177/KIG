...
def set_args():
    ...
    parser.add_argument('--test_num', default=500, type=int, help='测试数量')
...
def predict_one_sample(model, prompt_head, tokenizer, device, args, content, claim_label2 = None):
    ...
    inputs_embeds, attention_mask, position_ids, token_type_ids  = prompt_head(input_ids=input_tensors, \
                                                                    token_type_ids=token_type_tensors, claim_label2 = claim_label2)
    with torch.no_grad():
        # 遍历生成标题最大长度
        for _ in range(args.generate_max_len):

            transformer_outputs = model.model.transformer(inputs_embeds=inputs_embeds,
            ...
            inputs_embeds = torch.cat([inputs_embeds, model.embeddings(next_tokens)], dim=1)
            attention_mask = torch.cat((attention_mask, next_attention_mask), dim=-1)
            next_pos_ids = torch.tensor([[inputs_embeds.shape[1]]]).long().to(device)
...
def main():
    ...
    if args.context_aware:
        prompt_head = PromptHead(args, model.model, model.prompt_encoder, model.premodel)
    else:
        prompt_head = PromptHead(args, model.model, model.prompt_encoder)
