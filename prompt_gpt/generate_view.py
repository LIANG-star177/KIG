...
def set_args():
    ...
    parser.add_argument('--prompt_model_path', default='log/multi_prompt_saved_model/checkpoint-20840', type=str, help='模型输出路径')
...
def predict_one_sample(model, tokenizer, device, args, content, claim_label2 = None):
    """
    对单个样本进行预测
    Args:
        model: 模型
    ...
    with torch.no_grad():
        ...
        for _ in range(args.generate_max_len):
            if args.multi_prompt:
                outputs = model(input_ids=input_tensors, token_type_ids=token_type_tensors, claim_label2 = claim_label2)
            else:
